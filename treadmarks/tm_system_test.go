...
func TestShouldGetCopyIfNoCopy(t *testing.T) {
	...
	assert.True(t, managerHost.HasCopy(1))
...
func TestCreationAndPropagationOfWriteNotices(t *testing.T) {
	...
	assert.Equal(t, Vectorclock{Value: []uint{0, 1, 0, 0}}, host1.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).Timestamp)
	assert.Len(t, host1.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).WriteNotices, 1)
	...
	assert.Equal(t, Vectorclock{Value: []uint{0, 1, 0, 0}}, host2.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).Timestamp)
	assert.Equal(t, Vectorclock{Value: []uint{0, 1, 1, 0}}, host2.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).Timestamp)
	assert.Len(t, host2.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).WriteNotices, 1)
	assert.Len(t, host2.GetIntervalRecord(byte(1), Vectorclock{Value: []uint{0, 1, 0, 0}}).WriteNotices, 3)
...
func TestBarrierReadWrites(t *testing.T) {
	...
	assert.Equal(t, Vectorclock{Value: []uint{0, 0, 1, 0}}, host1.GetWritenoticeRecords(byte(2), 1)[0].GetTimestamp())
